...
A browser-based label designer and printer for the **Brother QL-700**, using **WebUSB** â€” no Brother drivers or P-touch software required. Design multi-element labels (text, symbols, images, shapes), use templates with variable data, print with cut lines, and drive the printer straight from Chrome.
...
- **Python 3** (only used to serve the files over `localhost` â€” WebUSB requires a secure context, so `file://` will not work).
- On **Windows**, the printer's USB interface must use the **WinUSB** driver (one-time Zadig setup â€” see below).

No build step, no `npm install` â€” it's plain HTML/CSS/ES modules.
...
2. **Install WinUSB with [Zadig](https://zadig.akeo.ie):**
   ...
   - Select **QL-700** â€” confirm the USB ID reads `04F9 2042` (not `2049`, which is the flash-drive mode).
...
> âš  Installing WinUSB replaces Brother's driver, so Brother's own software won't print until you revert it (Device Manager â†’ uninstall the QL-700 device with "delete driver", then re-run Brother's installer).
...
- **62 mm / 29 mm continuous** â€” endless tape, drag the label's long edge to set length.
- **29Ã—90 mm address / 62Ã—100 mm shipping (die-cut)** â€” fixed-size labels; length is locked.
...
- Text, ~100 Unicode symbols, images (auto Floydâ€“Steinberg dithering), and shapes (rect, rounded rect, circle) with solid or dithered-texture fills and text knockout/invert.
- Rulers, grid, guides (drag from a ruler), snap-to-grid / snap-to-guides, zoom.
- **Cut lines** â€” split a label into segments that print and cut in sequence.
- **Templates** â€” put `{{variables}}` in text, fill a data table (or bulk "Free edit" with `|` separators), and batch-print one label per row.
- **Import shipping PDF** â€” `File â†’ Import shipping PDFâ€¦` reads the address page (page 2) of an eBay-style shipping/packing PDF and builds one label per file: the **Send from** block, a cut, then the **Deliver to** block. Select several PDFs at once to import them all, then use **Print selected labels**.
...
- After editing any `.js` module, **hard-reload** the page (Ctrl+Shift+R) â€” browsers cache ES modules aggressively.
